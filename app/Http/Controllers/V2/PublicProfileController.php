...
class PublicProfileController extends Controller
{
    ...
    private function showProfile(string $modelClass, string $resourceClass, int $id, string $message): JsonResponse
    {
        ...
        $receiverType = $this->invitationReceiverTypeForProfileModel($modelClass);
        if ($modelClass === OrganiserV2::class) {
            $this->organiserProfileEventService->hydrateUpcomingPublicEventsOnProfiles([$profile]);
            $this->organiserProfileEventService->hydratePastPublicEventsOnProfiles([$profile]);
        } elseif ($receiverType !== null) {
